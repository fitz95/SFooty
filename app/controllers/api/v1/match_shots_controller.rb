class Api::V1::MatchShotsController < ApplicationController
  ...
  def index
    @match_shots = @match.match_shots.where(user_id: @user.id)
  ...
  def on_target
    @on_target_shots = MatchShot.on_target.where(match_id: params[:match_id], user_id: @user.id)
    render json: @on_target_shots, status: :ok
  end

  # GET /api/v1/users/:user_id/matches/:match_id/match_shots/missed?team_id=:team_id
  def missed_shots
    missed_shots_count = MatchShot.missed.where(match_id: params[:match_id], team_id: params[:team_id], user_id: @user.id).count
    render json: { missed_shots: missed_shots_count }, status: :ok
  end

  # GET /api/v1/users/:user_id/matches/:match_id/match_shots/total_xg
  def total_xg
    total_xg = MatchShot.on_target.where(match_id: params[:match_id], user_id: @user.id).sum(:expected_goal_value)
  ...
  def create
    ...
    @shot.user_id = @user.id # Ensure the shot is associated with the user
  ...
  def set_match_shot
    @match_shot = @match.match_shots.find_by(id: params[:id])
    if @match_shot.nil?
      render json: { error: 'Match shot not found for the user' }, status: :not_found
    end
  end
